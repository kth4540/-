		...
		else if (turn == 1)
		{
			for (int j = 0; j < 5; ++j)
			{
				if (pow(blue_ball[i].x - white_ball[j].x, 2) + pow(blue_ball[i].y - white_ball[j].y, 2) < 100)
				{
					...
					white_ball[j].cos = ((blue_ball[i].y - white_ball[j].y) / 10);
					white_ball[j].sin = ((blue_ball[i].x - white_ball[j].x) / 10);
					white_shot_check[j] = true;
		
				}
				if (pow(white_ball[i].x - blue_ball[j].x, 2) + pow(white_ball[i].y - blue_ball[j].y, 2) < 100)
				{
					blue_ball[j].force = white_ball[j].force;
					blue_ball[j].cos = -cos(PI - (t_angle + (2 * blue_ball[j].angle)));
					blue_ball[j].sin = -sin(PI - (t_angle + (2 * blue_ball[j].angle)));
					blue_shot_check[j] = true;
				}
			}
		}
